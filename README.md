...
- **A mature install stays small.** The one installation with real age, 91 entries over 125 days, had a STATUS of 42 lines. The template is about 37.
...
- **`SKILL.md`** — the operating instructions: when to trigger, and the full walkthrough for each of the three modes.
...
- **`docs/`** — the skill's own registers. It runs the protocol it ships, so its STATUS, CHANGELOG and DECISIONS are a live example of the format, and `EVIDENCE.md` holds the anonymised audit figures.
